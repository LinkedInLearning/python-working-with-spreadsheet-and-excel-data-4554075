...
summary_data = [
    ["Product","Gross Sales","Profits"]
]
for prod_name in df_sales["Product"].unique():
    sales_total = df_sales.loc[df_sales['Product'] == prod_name, "Gross Sales"].sum()
    profit_total = df_sales.loc[df_sales['Product'] == prod_name, "Profit"].sum()
